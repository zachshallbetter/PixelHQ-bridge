...
export class PixelOfficeBridge {
  ...
  preflight(): PreflightResult {
    ...
    const hasCursor = config.cursorDir && existsSync(config.cursorDir);

    if (!hasClaude && !hasCursor) {
      throw new Error(
        `No supported agents found. Tried:\n` +
        `  - Claude Code: ${config.claudeDir}\n` +
        `  - Cursor: ${config.cursorDir || 'not configured'}`
      );
    }
  ...
  async start(): Promise<void> {
    try {
      ...
      if (config.cursorDir && existsSync(config.cursorDir)) {
        logger.verbose('Bridge', `Cursor: ${config.cursorDir} (${config.cursorDirResolvedVia || 'auto'})`);
      }
      const watchPaths = [config.projectsDir];
      if (config.cursorSessionsDir) watchPaths.push(config.cursorSessionsDir);
      logger.verbose('Bridge', `Watching: ${watchPaths.join(', ')}`);
